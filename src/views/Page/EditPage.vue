        ...
        <div class="flex flex-col md:flex-row gap-8 items-center md:gap-16 md:mx-10 md:my-5 overflow-y-auto scrollbar-none">
          ...
          <div class="md:w-1/2 h-full flex justify-end items-center md:overflow-y-auto pr-2">
            <div class="flex flex-col justify-between h-full w-full">
                    ...
                    <div>
                      <div class="dropdown dropdown-top">
                        ...
                        <div tabindex="0" @click.stop="" class="h-64 w-64 overflow-y-scroll dropdown-content mt-2 p-2 bg-gray-100 dark:bg-gray-800 dark:border-gray-600 border rounded-md shadow-xl">
                          <CategoriesList @selected="emittedCategories" :allSelected="false" :toSelect="pageData.categories" />
                        </div>
                      </div>
                    </div>
                  </div>
              ...
              <div @click="updatePage" class="w-full mt-7 bg-sky-600 lg:hover:bg-sky-700 text-white p-2 rounded-md cursor-pointer transition" :class="{ 'cursor-not-allowed bg-sky-900 lg:hover:bg-sky-900': isSaving }">
  ...
  methods: {
    ...
    async updatePage() {
      ...
      if(this.pName != this.pageData.name) {
        await changePageName(this.pageData.pid, this.pName);
      }

      if(this.pDescription != this.pageData.description) {
        await updatePageDescription(this.pageData.pid, this.pDescription);
      }

      if(this.pCategories != this.pageData.categories) {
        await updatePageCategories(this.pageData.pid, this.pCategories);
      }

      alert("Pomyślnie zaktualizowano stronę");
      this.$parent.toggleEditPage();
    },
  },
  mounted() {
